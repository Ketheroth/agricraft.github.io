...
```
`damp` has a value of 2 (its index in [this chart](soil#humidity)).
...
You can use [this tool](requirements-calculator) to determine the tolerance factor for each soil condition.
...
```
- `crop_rhombus`: 4 faces spanning between the centers of the block faces, only used for weeds.

[//]: # (TODO: @Ketheroth do this when weeds are done)
